...
$(function () {
	$.ajax({
		...
		success: function (data) {
			...
			$('#immediateStorage').text(data.data[0].immediateLevel);
			$('#immediateStorage').attr('value',data.data[0].immediateLevel.replace(',',''));
			$('#selectReservoir').change(function(){
				$('#immediateStorage').text(data.data[$(this).val()].immediateLevel);
				$('#immediateStorage').attr('value',data.data[$(this).val()].immediateLevel.replace(',',''));
				x=parseFloat($('#immediateStorage').attr('value'));
				y=parseFloat($('#domesticConsumption').attr('value'));
				console.log(x+' '+y);
				$('#restDays').text(parseInt(x*0.18*10000/(y/365)));
