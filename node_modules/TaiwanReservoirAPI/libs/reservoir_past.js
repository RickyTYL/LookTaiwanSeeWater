...
module.exports = function(callback, pastDate) {
    ...
    async.waterfall([
        ...
        function(html, cb) {
            ...
            var time = moment().subtract(pastDate, 'days');
            var $ = cheerio.load(html);

            var form = $('#form1');
            // 選擇今天日期

            form.find('select#cphMain_ucDate_cboYear').val(time.year());
            form.find('select#cphMain_ucDate_cboMonth').val(time.month() + 1);
            form.find('select#cphMain_ucDate_cboDay').val(time.date());
            ...
            var URIComponent = decodeURIComponent(script.attr('src')).match(/;;AjaxControlToolkit.*/);
            if( !(URIComponent instanceof Array) )
              return cb('decodeURIComponent(script.attr("src")).match(/;;AjaxControlToolkit.*/) is null');

            var ctl02_value = URIComponent[0];
        ...
        function(html, cb) {
            ...
            $('.list').find('tr').each(function(i, elem) {
                ...
                var level = td.eq(8).text().trim().replace(',', '');
                var storage = td.eq(9).text().trim().replace(',', '');
                var percentage = td.eq(10).text().trim().replace(' %','');
